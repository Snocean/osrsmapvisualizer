...
class PinMapperApp:
    ...
    def resize_background(self, event=None):
        ...
        for line in self.lines:
            if line["pin"] and "rel_pos" in line:
                ...
                # Update pin coordinates (triangle, oval, etc.)
                for line in self.lines:
                    if line["pin"] and "rel_pos" in line:
                        rel_x, rel_y = line["rel_pos"]
                        x = rel_x * canvas_width
                        ...
                        line["last_pos"] = (x, y)
                # Also update last_pos in pixels
                line["last_pos"] = (x, y)
    ...
    def add_line(self, text=None, checked=True, color="#FFFF00", pin_pos=None):
        ...
        # Function to show the menu at cursor
        def show_menu(event):
            menu.tk_popup(event.x_root, event.y_root)

        # Bind right-click to the frame AND all child widgets
        widgets_to_bind = [frame, chk, color_box, lbl]
        for w in widgets_to_bind:
            w.bind("<Button-3>", show_menu)

        def change_color(event, line_data=line_data, color_box=color_box):
        # Open color chooser dialog
            new_color = colorchooser.askcolor(title="Choose pin color", color=line_data["color"])
            if new_color and new_color[1]:
                line_data["color"] = new_color[1]  # hex string
                color_box.config(background=new_color[1])
                # Update pin color if it exists
                if line_data["pin"]:
        ...
        if checked:
            self.create_pin(line_data, pin_pos)
    ...
    def make_draggable(self, pin, line_data):
        ...
        def do_drag(event):
            ...
            coords = self.canvas.coords(pin)  # [x1,y1, x2,y2, x3,y3]
            x = sum(coords[0::2]) / 3
            y = sum(coords[1::2]) / 3
    ...
    def load_state(self):
        ...
        for item in state:
            ...
            # Store the relative position in the line_data (so resizing works)
            if item.get("rel_pos"):
                line_data["rel_pos"] = tuple(item["rel_pos"])
            elif item.get("pin_pos"):
                # Convert absolute pixel to relative coordinates
                canvas_width = self.canvas.winfo_width()
                canvas_height = self.canvas.winfo_height()
                if canvas_width > 0 and canvas_height > 0:
                    line_data["rel_pos"] = (item["pin_pos"][0] / canvas_width, item["pin_pos"][1] / canvas_height)
                else:
                    line_data["rel_pos"] = (0.5, 0.5)  # fallback

            # Schedule pin creation after the canvas has proper size
            if line_data["var"].get():  # only create if checked
                self.root.after(50, lambda ld=line_data: self.create_pin(ld, pos=(
                    ld["rel_pos"][0] * self.canvas.winfo_width(),
                    ld["rel_pos"][1] * self.canvas.winfo_height()
                )))
